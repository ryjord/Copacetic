...
const { showNewTabMenu, showTabContextMenu } = await import('../electron/main/context-menu');
...
function fakeBrowser({ hasClosedTabs = true, url = 'https://example.com' } = {}) {
  ...
  return {
    ...
    store: { isBookmarked: () => false, toggleBookmark: record('toggleBookmark') },
    tabs: {
      urlFor: () => url,
      titleFor: () => 'Example',
...
describe('every item in both menus', () => {
  ...
  it('reaches for methods that exist', () => {
    const menuSource = readFileSync('electron/main/context-menu.ts', 'utf8');
    const sources = {
      'tabs.': readFileSync('electron/main/tabs.ts', 'utf8'),
      'store.': readFileSync('electron/main/store.ts', 'utf8'),
      '': readFileSync('electron/main/browser.ts', 'utf8'),
    };
